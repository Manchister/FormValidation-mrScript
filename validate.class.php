...
// Validate email
        ...
        /* /*Validate password
          if (!$this->validatePassword($_POST['pass1'])) {
          $_SESSION['errors']['pass1'] = 'error';
          $errorsExist = 1;
          }
          if (!$this->validatePassword($_POST['pass2'])) {
          $_SESSION['errors']['pass2'] = 'error';
          $errorsExist = 1;
          }
         */
...
// If no errors are found, point to a successful validation page
        if ($errorsExist == 0) {
            ...
            $bd = $_POST['txtBthYear'] . '-' .
                    ...
                    $_POST['txtBthDay'];
            $ps = sha1($_POST['pass1']);
            $allValues = "'" . $un . "', '" . $em . "', '" . $na . "', '" . $gn . "', '" . $bd . "', '" . $ps . "'";
            $colOrder = 'user_name, email, name, gender, birth_date, pass';
            $fullQuery = "INSERT INTO users (user_name, email, name, gender, birth_date, pass) VALUES (" . $allValues . ")";
            
            $connectPDO = new DbAccess();
            $connectPDO->query($fullQuery);
            return 'allok.php';
        } else {
...
/*        $query = $this->mMysqli->query('SELECT email FROM users ' .
                'WHERE email="' . $value . '"');
        if ($this->mMysqli->affected_rows > 0) {
            return 0;
        } // not valid
 ...
 */
    ...
    /* /* validate password
      ...
      return ($_SESSION['values']['Pass1'] == '' || $value == $_SESSION['values']['Pass1']) ? 0 : 1;
      }
     */
